...
function orderTable(list){if(!list.length)return "<p>Nenhum pedido encontrado.</p>";return `<table class="table order-table"><thead><tr><th>PEDIDO</th><th>CLIENTE</th><th>DATA</th><th>TOTAL</th><th>STATUS</th><th></th></tr></thead><tbody>${list.map(o=>`<tr><td><button class="link-button" onclick="openOrder('${o.id}')">${esc(o.order_code||o.order_nsu||"—")}</button></td><td>${esc(o.customer_name||"—")}</td><td>${date(o.created_at)}</td><td>${money(o.total)}</td><td><span class="badge ${esc(o.status||"pending")}">${statusLabel(o.status)}</span></td><td><button class="secondary small-button" onclick="openOrder('${o.id}')">DETALHES</button></td></tr>`).join("")}</tbody></table>`}
...
function openOrder(id){const o=orders.find(x=>String(x.id)===String(id));if(!o)return;$("orderModalTitle").textContent=o.order_code||o.order_nsu||"Pedido";$("orderDetails").innerHTML=orderDetail(o);$("orderModal").classList.remove("hidden")}
...
async function deleteProduct(id){const product=products.find(p=>String(p.id)===String(id));if(!product)return;const confirmed=window.confirm("Excluir o produto \""+(product.name||"produto")+"?\"\n\nEsta ação remove o produto do catálogo. Produtos que já possuem pedidos não podem ser excluídos; nesse caso, use DESATIVAR.");if(!confirmed)return;try{await api("/api/admin/products/"+encodeURIComponent(id),{method:"DELETE"});await loadProducts()}catch(e){window.alert(e.message)}}
