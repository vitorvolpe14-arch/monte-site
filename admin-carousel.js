...
function bindCarouselImageDropzone(){
  if(carouselModuleBound)return;

  const zone=$("carouselImageDropzone");
  const input=$("carouselImageFiles");
  const button=$("selectCarouselPhotosButton");
  const saveButton=$("saveCarouselButton");
  if(!zone||!input)return;

  carouselModuleBound=true;

  if(saveButton)saveButton.onclick=saveCarousel;
  ...
  zone.onclick=e=>{
    if(e.target!==input&&e.target!==button)input.click();
  };
  ...
  input.onchange=async e=>{
    await handleCarouselFiles([...e.target.files]);
    input.value="";
  };
  ...
  zone.addEventListener("drop",async e=>{
    await handleCarouselFiles([...e.dataTransfer.files]);
  });
}
...
async function handleCarouselFiles(files){
  ...
  try{
    ...
    setCarouselStatus(e.message,true);
  }
}
